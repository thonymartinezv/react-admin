...
export function Sidebar({ links, name }) {
    if (links) {
        return (
...
function LinkSidebar({ name, to, icon }) {
    var Icon = icon
    ...
    return (
        <Link
            to={to}
            className={"list-group-item list-group-item-action bg-dark " + (active ? "activo" : "")
            }>
            <div className="row ml-2">
                <Icon className={"mr-2 icon-sidebar " + (active ? "activo" : "")} />
...
export function Navbar({ toggled }) {
    return (
        <nav className="navbar navbar-expand-lg navbar-light bg-light border-bottom" id="navbar">
            <button className="btn btn-light border border-gray" onClick={toggled} id="menu-toggle">
